...
typedef int32_t time_tp;     // timestamp or interval in microseconds, timestamps have a fixed
...
typedef int32_t count_tp;    // count in packets (or frames), signed because it
                             // can wrap around, and we need to compare both ways
...
struct PragueState {
    ...
    // both-end variables
    time_tp m_ts_remote;            // to keep the frozen timestamp from the peer, and
                                    // echo it back defrosted
    ...
    bool m_r_error_L4S;  // as a receiver, check L4S-ECN validity to echo back
...
class PragueCC : private PragueState {
   public:
    PragueCC(size_tp max_packet_size = PRAGUE_INITMTU,  // use MTU detection, or a low enough value.
                                                        // Can be updated on the fly (todo)
             fps_tp fps = 0,                            // only used for video; frames per second, 0 must
                                                        // be used for bulk transfer
             time_tp frame_budget = 0,                  // only used for video; over what time [µs] you want to
                                                        // pace the frame (max 1000000/fps [µs])
    ...
    time_tp Now();  // will have to return a monotonic increasing signed int 32
                    // which will wrap around (after 4000 seconds)

    bool PacketReceived(            // call this when a packet is received from peer,
                                    // returns false if the old packet is ignored
    ...
    bool ACKReceived(               // call this when an ACK is received from peer, returns
        ...
        count_tp packets_sent,      // local counter of packets sent up to now, an
                                    // RTT is reached if remote ACK
                                    // packets_received+packets_lost
        bool error_L4S,             // receiver found a bleached/error ECN; stop using
                                    // L4S_id on the sending packets!
    ...
    void DataReceived(           // call this when a data packet is received as a
    ...
    void DataReceivedSequence(    // call this every time when a data packet with
                                  // a sequence number is received as a receiver
        ecn_tp ip_ecn,            // IP.ECN field value
    ...
    void GetCCInfo(               // when the sending-app needs to send a packet
        ...
        count_tp &packet_burst,   // number of packets that can be paced at once
                                  // (<250µs)
