...
typedef uint64_t window_tp;  // fractional window size in µBytes (to match time in µs, for easy
                             // Bytes/second rate calculations)
typedef uint64_t rate_tp;    // rate in Bytes/second
typedef int32_t time_tp;  // timestamp or interval in microseconds, timestamps have a fixed but no
                          // meaningful reference, so use only for intervals beteen 2 timestamps
                          // signed because it can wrap around, and we need to compare both ways (<
                          // 0 and > 0)
typedef int32_t count_tp;  // count in packets (or frames), signed because it can wrap around, and
...
typedef uint8_t
    fps_tp;  // frames per second: any value from 1 till 255 can be used, 0 must be used for bulk
...
class PragueCC : private PragueState {
   public:
    PragueCC(
        size_tp max_packet_size = PRAGUE_INITMTU,  // use MTU detection, or a low enough value. Can
                                                   // be updated on the fly (todo)
        fps_tp fps = 0,  // only used for video; frames per second, 0 must be used for bulk transfer
        time_tp frame_budget = 0,  // only used for video; over what time [µs] you want to pace the
                                   // frame (max 1000000/fps [µs])
        rate_tp init_rate = PRAGUE_INITRATE, count_tp init_window = PRAGUE_INITWIN,
        rate_tp min_rate = PRAGUE_MINRATE, rate_tp max_rate = PRAGUE_MAXRATE);

    ~PragueCC();

    time_tp Now();  // will have to return a monotonic increasing signed int 32 which will wrap
                    // around (after 4000 seconds)

    bool PacketReceived(  // call this when a packet is received from peer, returns false if the old
                          // packet is ignored
    ...
    bool ACKReceived(  // call this when an ACK is received from peer, returns false if the old ack
                       // is ignored
        ...
        count_tp packets_sent,      // local counter of packets sent up to now, an RTT is reached if
                                    // remote ACK packets_received+packets_lost
    ...
    void DataReceived(  // call this when a data packet is received as a receiver and you can
                        // identify lost packets
        ecn_tp ip_ecn,  // IP.ECN field value
        count_tp packets_lost);  // packets skipped; can be optionally -1 to potentially undo a
                                 // previous cwindow reduction

    void DataReceivedSequence(  // call this every time when a data packet with a sequence number is
