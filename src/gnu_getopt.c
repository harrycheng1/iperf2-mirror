...
static const char *_gnu_getopt_initialize(int argc, char *const *argv, const char *optstring);
...
#define NONOPTION_P (argv[gnu_optind][0] != '-' || argv[gnu_optind][1] == '\0')
    ...
    if (nextchar == NULL || *nextchar == '\0') {
       ...
       moved back by the user (who may also have changed the arguments).  */
        if (last_nonopt > gnu_optind) last_nonopt = gnu_optind;
        ...
        if (ordering == PERMUTE) {
            ...
            while (gnu_optind < argc && NONOPTION_P) gnu_optind++;
            last_nonopt = gnu_optind;
        }
       ...
       and back over any non-options that we skipped and permuted.  */
        ...
        if (gnu_optind == argc) {
            ...
            if (first_nonopt != last_nonopt) gnu_optind = first_nonopt;
            return -1;
        }
       ...
       Skip the initial punctuation.  */
        ...
        nextchar = (argv[gnu_optind] + 1 + (longopts != NULL && argv[gnu_optind][1] == '-'));
    }
    ...
    if (longopts != NULL && (argv[gnu_optind][1] == '-' ||
       ...
       Otherwise interpret it as a short option.  */
        if (!long_only || argv[gnu_optind][1] == '-' || my_index(optstring, *nextchar) == NULL) {
            if (gnu_opterr) {
                if (argv[gnu_optind][1] == '-') /* --option */
...
int gnu_getopt(int argc, char *const *argv, const char *optstring) {
    return _gnu_getopt_internal(argc, argv, optstring, (const struct option *)0, (int *)0, 0);
}
